...
public class BBSController {
    ...
    @Autowired
    BBSVerLogService bbsVerLogService;
     ...
     * @return
     */
    @RequestMapping("/bbs/topic/addVL.html")
    public ModelAndView addVL(ModelAndView view) {
        String TLOVLName;
        // 获得上一版本
        if (bbsVerLogService.getTheLastOneVL().size() == 0) {
            TLOVLName = "暂无";
        } else {
            TLOVLName = bbsVerLogService.getTheLastOneVL().get(0).getVerLogName();
        }
        view.addObject("TLOneVLName", TLOVLName);
        view.setViewName("/postVL.html");
        return view;
    }
...
//		}
     ...
     */
    @ResponseBody
    @PostMapping("/bbs/topic/saveVL")
    public JSONObject saveVerLog(BbsVerLog bbsVerLog, String vlName, String vl1, String vl2, String vl3, String vl4,
                                 String vl5, String vl6, String vl7, String vl8, String vl9, String vl10, String vl11, String vl12,
                                 String vl13, String vl14, String vl15, HttpServletRequest request, HttpServletResponse response) {
        JSONObject result = new JSONObject();
        result.put("err", 1);
        if (webUtils.currentUser(request, response) == null) {
            result.put("msg", "请先登录后再继续！");
        } else if (StringUtils.isEmpty(vlName) || StringUtils.isEmpty(vl1)) {
            result.put("msg", "版本标题、更新内容必输入！且从NO.1开始！");
        } else {
            bbsVerLog.setVerLogName(vlName);
            bbsVerLog.setVerLog1(vl1);
            bbsVerLog.setVerLog2(vl2);
            bbsVerLog.setVerLog3(vl3);
            bbsVerLog.setVerLog4(vl4);
            bbsVerLog.setVerLog5(vl5);
            bbsVerLog.setVerLog6(vl6);
            bbsVerLog.setVerLog7(vl7);
            bbsVerLog.setVerLog8(vl8);
            bbsVerLog.setVerLog9(vl9);
            bbsVerLog.setVerLog10(vl10);
            bbsVerLog.setVerLog11(vl11);
            bbsVerLog.setVerLog12(vl12);
            bbsVerLog.setVerLog13(vl13);
            bbsVerLog.setVerLog14(vl14);
            bbsVerLog.setVerLog15(vl15);
            bbsVerLogService.saveVerLog(bbsVerLog);
            result.put("err", 0);
